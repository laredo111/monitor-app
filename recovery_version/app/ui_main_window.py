...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # קורא config.json לתכונות recovery mode
        ...
        if config_path.exists():
            try:
                with config_path.open("r", encoding="utf-8") as f:
                    config = json.load(f)
                    self.recovery_mode = config.get("recovery_mode", {}).get("enabled", False)
                    if self.recovery_mode:
                        self.logger.info("✅ RECOVERY MODE ENABLED - התראות יסתמנו בירוק 7 דקות")
            except Exception as e:
                self.logger.warning("Unable to load config.json: %s", e)

        # Core state
        self.core = MonitorCore(down_after=3, up_after=2, recovery_mode=self.recovery_mode)
    ...
    def _on_ping_results(self, results: list) -> None:
        ...
        rows = [
            AlertRow(
                ...
                is_recovered=getattr(a, "is_recovered", False),
            )
